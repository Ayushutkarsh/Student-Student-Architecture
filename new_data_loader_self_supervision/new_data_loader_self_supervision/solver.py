...
class Solver(object):
    def __init__(self, config, train_loader_rot,train_loader_hor,train_loader_ver, valid_loader_rot,valid_loader_hor,valid_loader_ver, test_loader):
        ...
        print("@@@@@@@@@@@@@@@@@@------> length of dataset",len(self.train_loader_rot),len(self.train_loader_ver))
        ...
        print("-------> Device being used is <-------", self.device)
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            length = 0
            for i, (data,label,task) in enumerate(tqdm(self.train_loader_rot)):
                ...
                for b, m in enumerate(self.train_loader_hor):
                    if b==i:
                        data,label,task=m[0],m[1],m[2]
                        break
                    else:
                        continue
                ...
                for b, m in enumerate(self.train_loader_ver):
                    if b==i:
                        data,label,task=m[0],m[1],m[2]
                        break
                    else:
            ...
            if epoch%20 ==0:
                ...
                self.unet.eval()
                for i, (data,label,task) in enumerate(tqdm(self.valid_loader_rot)):
